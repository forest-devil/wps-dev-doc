...
> 更多 API 用法可参考[多维表 API](/documents/app-integration-dev/guide/dbsheet/Api/api-instro.html)
...
1. 我们为您提供了一个 Demo 示例，建议下载使用，下载地址：[Demo 示例](https://365.kdocs.cn/view/l/cb11ZL018X1g)
...
WebOfficeSDK.config({
  ...
  url:
    url + "?disablePlugins&" + (fullScreen ? "embed-fullscreen=1" : "embed=1"),
  viewMode: "Embed",
  mode: "embed",
  fileType: "d",
  mount: container,
  ...
  dbOptions: {
    ...
    // 视图菜单白名单
    viewMenuFeatureWhiteList: ["rename", "addDesc", "deleteView", "copyView"],
    // 附件类型白名单
...
| fileType      | string      | 文件类型，WPS 多维表格使用 `d`                                                                                |
| commonOptions | object      | 组件通用配置，详见 commonOptions                                                                              |
| dbOptions     | object      | WPS 多维表格自定义配置，详见 dbOptions                                                                        |
...
# 054 内嵌使用 SDK*基础用法*高级用法

本页内容

# 高级用法 ​

更多高级用法可参考[多维表 API](/documents/app-integration-dev/guide/dbsheet/Api/api-instro.html)

# 055 内嵌使用 SDK\_界面定制案例
...
> 通过 SDK 监听 WPS 多维表格 内的事件，进而执行相关操作
...
> 当外部视图宽度达到最大的时候，需借助 WPS 多维表格的左右滚动查看数据

下面给出一个实现左右滚动的示例：在 React 代码中添加对 wheel 事件的监听，在事件处理逻辑里操控 WPS 多维表格 实现左右滚动。
...
WPS 多维表格为开发者提供了一套功能完备的聚合 API 体系，借助配套的 [SDK](/documents/app-integration-dev/guide/dbsheet/Weboffice/weboffice-instro.html)，开发者既能够在浏览器环境中灵活调用 API 开展项目开发，也能依托 [在线脚本 AirScript](/documents/app-integration-dev/guide/dbsheet/AirScript/AirScript-instro.html) 运用 API 来编写脚本，拓展业务功能。 丰富多样的 API 接口，赋予了用户极大的自主开发空间，扩展多维表的功能，定制个性化功能。

## 怎么使用 API ​

> 以快速创建一张表为例，下面分别是在 **浏览器环境** 和 **AirScript 脚本编辑器** 中的使用示例
...
async function example() {
  ...
  await app.Sheets.Add(null, 1, "xlEtDataBaseSheet", {
    fields: [
      {
        fieldType: "SingleLineText",
        args: { fieldName: "文本", fieldWidth: 15 },
...
```javascript
function main() {
  Application.Sheets.Add(1, null, "xlEtDataBaseSheet", {
    fields: [
      {
        fieldType: "SingleLineText",
        args: { fieldName: "文本", fieldWidth: 15 },
...
| 参数名 | 是否必需 | 类型          | 描述                                                                                                    |
| ------ | -------- | ------------- | ------------------------------------------------------------------------------------------------------- |
| Before | 否       | number/string | 插入到 Before(索引从 1 开始/数据表名)对应 sheet 之前                                                    |
| After  | 否       | number/string | 插入到 After(索引从 1 开始/数据表名)对应 sheet 之后                                                     |
...
| 属性名 | 是否必需 | 类型    | 描述                                                                                                                                                                                                                                                                       |
| ------ | -------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| fields | 否       | Field[] | 字段数组，Field 类型结构：{fieldType: FieldType,args: { fieldName: string, fieldWidth: number, listItems?: { value: string, color: number}[], numberFormat?: string, maxRating?: number } }                                                                                |
| name   | 否       | string  | 数据表名,默认为‘data1’                                                                                                                                                                                                                                                     |
| views  | 否       | View[]  | 视图配置数组，View 结构：{name: string,type: ViewType}，ViewType 的取值为：'Grid'（网格视图）、'Kanban'（看板视图）、'Gallery'（相册视图）、'Form'（表单视图）、'Gantt'（甘特视图）、'Query'（查询视图）或'Calendar'（日历视图）；默认创建'Grid'。暂只支持'Grid'和'Form'。 |
...
| 参数名 | 是否必需 | 类型    | 描述                                                                                                               |
| ------ | -------- | ------- | ------------------------------------------------------------------------------------------------------------------ |
| Value  | 否       | boolean | 创建副本的方式，默认为 false。传 true 复制全部内容；传 false 仅复制空表和视图；复制的 sheet 为仪表盘时，此参数不传 |
...
时间线颜色，值为 Hex 格式的 RGB 颜色值，如：#FF0000。
...
查询视图的查询条件配置数组，可以将数组设置到 QueryFields 属性，查询条件的数据结构如下
...
事件线颜色，值为 Hex 格式的 RGB 颜色值，如：#FF0000。
...
| 参数名 | 是否必需 | 类型          | 描述                                                            |
| ------ | -------- | ------------- | --------------------------------------------------------------- |
| Before | 否       | Number/String | 在行记录之前添加，Number 时指定插入时索引，String 时指定插入 ID |
| After  | 否       | Number/String | 在行记录之后添加，Number 时指定插入时索引，String 时指定插入 ID |
| Count  | 否       | Number        | 一次插入 N 条数据，未指定时插入 1 条                            |
...
| 参数名 | 是否必需 | 类型          | 描述                  |
| ------ | -------- | ------------- | --------------------- |
| Index  | 是       | number/string | 索引从 1 开始/记录 ID |
...
| 参数名   | 是否必需 | 类型       | 描述                                                                 |
| -------- | -------- | ---------- | -------------------------------------------------------------------- |
| Filters  | 是       | Filter[]   | 筛选数据的条件组, 条件组里每个 Filter，可以包含多个筛选条件          |
| FilterOp | 否       | "And"/"Or" | 筛选数据条件组之间的关系，是同时满足还是只需要满足一条，默认值为 And |
...
| Op        | 否       | "And"/"Or"                                                                   | 筛选条件项组之间的关系，是同时满足还是只需要满足一条，默认值为 And |
...
各筛选规则独立地限制了 values 数组内最多允许填写的元素数，当 values 内元素数超过阈值时，该筛选规则将失效。“为空、不为空”不允许填写元素；“介于”允许最多填写 2 个元素；“指定值”允许填写 65535 个元素；其他规则允许最多填写 1 个元素 values[]数组内的元素为字符串时，表示文本匹配。目前还支持对日期进行动态筛选，此时 values[]内的元素需以结构体的形式给出：
...
async function example() {
  ...
  filters.push(filter);
  // 筛选记录
  const res = await app.ActiveSheet.Views(1).RecordRange.Condition(
    filters,
    "AND"
  );
  console.log(res);
...
function main() {
  ...
  filters.push(filter);
  const res = Application.ActiveSheet.Views(1).RecordRange.Condition(
    filters,
    "AND"
  );
...
# 098 API 文档*记录*设置单元格内容

本页内容

- 说明
- 返回值
  - 这里返回的文本根据区域会有三种情况,RecordRange 选择器参看 RecordRange 的说明
  - 设置值的时候，也有三种设置值的方式
  - 不同字段类型设置 Value 时的数据结构
  - 部分不支持设置值的字段类型
- 浏览器环境示例
- 脚本编辑器 示例
...
3、传入二维数组，如果二维数组的长度为 1，目标是一条记录，则将二维数组[0]设置到目标区域，如果目标是多条记录，则会将相同的数据设置到所有记录。如果二维数组为 M x N，则按顺序传入到目标区域。

```javascript
Application.ActiveView.RecordRange([1, 2], [1, 2]).Value = [
  ["1", "2"],
  ["3", "4"],
];
...
地址字段类型: 通过 DBCellValue() 生成字段的数据

```javascript
Application.Sheets(1).Views(2).RecordRange(1, "@地址").Value = DBCellValue({
  districts: ["广东省", "珠海市", "香洲区"],
  detail: "前岛环路xxxx号",
});
```

级联字段类型：通过 DBCellValue() 生成字段的数据

```javascript
Application.Sheets(1).Views(2).RecordRange(1, "@级联选项").Value = DBCellValue({
  districts: ["广东省", "珠海市", "香洲区"],
});
```
...
```javascript
Application.RecordRange(1, "@超链接").Value = Application.DBCellValue({
  address: "wps.cn",
  display: "wps",
});
```
...
Application.Sheets(1).Views(2).RecordRange(1, "@关联：数据表").Value =
  DBCellValue(["b", "V"]);
```
...
```javascript
Application.Sheets(1).Views(2).RecordRange([5, 6], 4).Value = [
  [DBCellValue(["未开始", "进行中"])],
  DBCellValue(["进行中"]),
];
```

联系人字段类型：直接传入联系人的 id，如果有多个联系人，可用","进行分割

```javascript
Application.Sheets(1).Views(2).RecordRange([5, 6], "@联系人").Value =
  "238777563";
```

图片与附件字段：可以传入包含 URL/base64 编码的图片/云文档 的数组，支持多个附件。 注意：由于脚本有运行时长限制，附件较大/或者较多时会导致超时，设置失败

```javascript
Application.Sheets(1).Views(2).RecordRange(1, "@图片和附件").Value =
...
返回当前 RecordRange 首个单元格的字体属性[Font](/documents/app-integration-dev/guide/dbsheet/Api/Font.html)
...
可读 返回当前 RecordRange 首个单元格的填充属性[Interior](/documents/app-integration-dev/guide/dbsheet/Api/Interior.html)
...
| Index           | 否       | string/number   | Index 为 string 时表示字段 ID，number 时表示字段索引，插入位置，未指定时插入到末尾 |
...
| 参数名 | 是否必需 | 类型          | 描述                       |
| ------ | -------- | ------------- | -------------------------- |
| Before | 否       | String/Number | 在 Before 之前插入复制字段 |
| After  | 否       | String/Number | 在 After 之后插入复制字段  |
...
| 参数名 | 是否必需 | 类型          | 描述                   |
| ------ | -------- | ------------- | ---------------------- |
| Before | 否       | String/Number | 移动到 Before 字段之前 |
| After  | 否       | String/Number | 移动到 After 字段之后  |
...
async function example() {
  ...
  const fieldDescriptor = await Application.Sheets("数据表").FieldDescriptors(
    "@日期"
  );
...
function main() {
  const fieldDescriptor =
    Application.Sheets("数据表").FieldDescriptors("@日期");
...
字段类型为 Number 时，通过此属性可以快捷的设置是否显示千位符
...
async function example() {
  ...
  const app = instance.Application;
  // 设置超链接
  app.Sheets(1).Views(1).RecordRange(2, "@超链接").Value =
    await Application.DBCellValue({
      address: "wps.cn",
      display: "wps",
    });
}
...
function main() {
  // 设置超链接
  Application.Sheets(1).Views(1).RecordRange(2, "@超链接").Value =
    Application.DBCellValue({
      address: "wps.cn",
      display: "wps",
    });
}
...
1. 单选项字段类型时，返回 `string` 类型，如 '选项 1'
2. 多选项字段类型时，返回`string[]`类型，如 ['选项 1', '选项 2']
...
async function example() {
  ...
  // 设置多选项
  app.Sheets(1).Views(1).RecordRange(2, "@多选项").Value = [
    [await Application.DBCellValue(["选项1", "选项2", "选项3"])],
  ];
}
```
...
function main() {
  ...
  // 设置多选项
  Application.Sheets(1).Views(1).RecordRange(2, "@多选项").Value = [
    [Application.DBCellValue(["选项1", "选项2", "选项3"])],
  ];
}
main();
```

# 125 API 文档*字段*单选项*多选项字段*设置选项
...
# 126 API 文档*字段*单选项*多选项字段*允许填写时添加选项
...
async function example() {
  await instance.ready();
  const app = instance.Application;
  const value = await app.Sheets(1).Views(1).RecordRange(1, "@图片与附件")
    .Value;
  console.log(value);
  /**
   * 输出值：
...
```javascript
function main() {
  const value = Application.Sheets(1)
    .Views(1)
    .RecordRange(1, "@图片与附件").Value;
  console.log(value);
}
main();
```

# 131 API 文档*字段*图片和附件字段\_设置字段值
...
| 参数名   | 是否必需 | 类型   | 描述                                    |
| -------- | -------- | ------ | --------------------------------------- |
| fileData | 是       | string | 支持 2 种类型：`base64` 或 `云文档链接` |
...
async function example() {
  ...
  const app = instance.Application;
  // 设置云文档类型
  app.Sheets(1).Views(1).RecordRange(2, "@图片和附件").Value =
    await Application.DBCellValue([
      {
...
function main() {
  // 设置云文档类型
  Application.Sheets(1).Views(1).RecordRange(2, "@图片和附件").Value =
    Application.DBCellValue([
      {
        fileData: "https://kdocs.cn/l/csGRGIzv9PvF",
        fileName: "11.png",
...
# 133 API 文档*字段*图片和附件字段\_仅可通过移动端拍摄上传
...
async function example() {
  ...
  app.Sheets(1).Views(1).RecordRange(2, "@联系人").Value =
    "1557445524,238777563";
}
```
...
function main() {
  // 设置联系人字段
  Application.Sheets(1).Views(1).RecordRange(2, "@联系人").Value =
    "1557445524,238777563";
}
main();
```

# 136 API 文档*字段*联系人字段\_插入多个联系人
...
function main() {
  const fieldDescriptor =
    Application.Sheets("数据表").FieldDescriptors("@联系人");
...
async function example() {
  ...
  app.Sheets(1).Views(1).RecordRange(3, "@关联").Value =
    await Application.DBCellValue(["G", "H"]);
}
```
...
function main() {
  // 设置关联字段值，参数为传入关联的记录id
  Application.Sheets(1).Views(1).RecordRange(2, "@联系人").Value =
    Application.DBCellValue(["G", "H"]);
}
main();
```

# 140 API 文档*字段*关联字段\_关联多条记录
...
async function example() {
  ...
  const app = instance.Application;
  // 设置地址字段值
  app.Sheets(1).Views(1).RecordRange(1, "@关联").Value =
    await Application.DBCellValue({
      districts: ["广东省", "珠海市", "香洲区"],
      detail: "前岛环路xxxx号",
    });
...
function main() {
  // 设置关联字段值，参数为传入关联的记录id
  Application.Sheets(1).Views(1).RecordRange(1, "@联系人").Value =
    Application.DBCellValue({
      districts: ["广东省", "珠海市", "香洲区"],
      detail: "前岛环路xxxx号",
    });
...
function main() {
  const fieldDescriptor =
    Application.Sheets("数据表").FieldDescriptors("@地址");
...
地址字段 是否预设指定地址,当设置了 IsUsePresetAddress 为 false 后，PresetAddress 不生效
...
地址字段的默认值,当设置了 IsUsePresetAddress 为 false 后，PresetAddress 不生效
...
function main() {
  const fieldDescriptor =
    Application.Sheets("数据表").FieldDescriptors("@公式");
...
自动任务字段的类型，有三种类型，参考枚举值 DbAutomationPresetType ，分别为 CheckedNotifyContact，UpdatedNotifyContact，DueDateNotifyContact
...
async function example() {
  await instance.ready();
  const Application = instance.Application;
  const contactField = await Application.FieldDescriptor(
    "Contact",
    "联系人字段"
  );
  ...
  const newField = await Application.FieldDescriptor(
    "Automations",
    "自动任务字段"
  );
...
async function example() {
  ...
  const app = instance.Application;
  // 设置级联字段值
  app.Sheets(1).Views(1).RecordRange(2, "@级联").Value =
    await Application.DBCellValue({
      districts: ["选项1", "选项1-1"],
    });
}
...
function main() {
  // 设置级联字段值
  Application.Sheets(1).Views(1).RecordRange(2, "@级联").Value =
    Application.DBCellValue({
      districts: ["选项1", "选项1-1"],
    });
}
...
function main() {
  const fieldDescriptor =
    Application.Sheets("数据表").FieldDescriptors("@级联选项");
...
# 160 API 文档*字段*最后修改人*最后修改时间字段*监听所有字段
...
function main() {
  const fieldDescriptor =
    Application.Sheets("数据表").FieldDescriptors("@最后修改时间");
...
# 161 API 文档*字段*最后修改人*最后修改时间字段*监听某些字段
...
最后修改人/最后修改时间，监听某些字段的修改情况, 如果需要监听某些特定的字段，需要将属性 IsWatchedAll 设置为 false, 否则这个设置不生效
...
| 参数名      | 是否必需 | 类型     | 描述                                                                        |
| ----------- | -------- | -------- | --------------------------------------------------------------------------- |
| FromField   | 是       | [string] | 要移动的排序字段的字段 ID/要移动的排序字段的字段名称(名称要以@字符作为开始) |
| BeforeField | 否       | [string] | 目标位置前的排序字段 ID/目标位置前的排序字段名称(名称要以@字符作为开始)     |
| AfterField  | 否       | [string] | 目标位置后的排序字段 ID/目标位置后的排序字段名称(名称要以@字符作为开始)     |

FromField、BeforeField 和 AfterField 必须都是已设置的排序条件字段，BeforeField 和 AfterField 至少需要传入一个，如果 BeforeField 和 AfterField 同时存在以 BeforeField 作为应用参数

比如表格视图中已设置的排序条件在排序面板中从上到下依次为【公式，日期，名称，数量】 现在想将名称这条排序条件移动到日期的前面，结果变为【公式，名称，日期，数量】，就可以用以下方式实现

```javascript
await WPSOpenApi.Application.Sheets(1)
  .Views(1)
  .Sorts.ChangeOrder("@名称", "@日期");
```
...
| 参数名      | 是否必需 | 类型          | 描述                                            |
| ----------- | -------- | ------------- | ----------------------------------------------- |
| Field       | 是       | number/string | 新增分组字段索引/新增分组字段 ID/新增分组字段名 |
...
jssdk.on("OnBroadcast", async (res) => {
  const data = res.Data;
  if (data.type == "DB_COMMENT_UPDATE") {
    // 收到文档评论更新消息
    if (data.shouldNotLocalUpdate) {
      ...
      if (action == "Add") {
        // 新增评论
        const addText = await jssdk.Application.Sheets.ItemById(sheetStId)
          .ActiveView.RecordComments(recordId)
          .Item(commentId).Text;
...
async function example() {
  ...
  eventContext = await app.ActiveView.RecordComments.OnCreate(async (data) => {
    ...
    if (action == "Add") {
      // 新增评论
      const addText = await app.Sheets.ItemById(sheetStId)
        .ActiveView.RecordComments(recordId)
        .Item(commentId).Text;
      console.log("新增评论：", addText);
    } else if (action == "Delete") {
...
async function example() {
  ...
  const app = instance.Application;
  let eventContext;
  eventContext = await app.ActiveView.RecordComments.OnDelete((data) =>
...
async function example() {
  await instance.ready();
  const app = instance.Application;
  const syncSheets = await app.DataSource.CreateSyncDBSheets(
    "100127684526",
    "d",
    [1]
  );
...
```javascript
function main() {
  const syncSheets = Application.DataSource.CreateSyncDBSheets(
    "100127684526",
    "d",
    [1]
  );
...
合并表配置对象中添加数据源（只是编辑的本地对象，需要调 Apply 方法才能更新到云上）
...
| 参数名 | 是否必需 | 类型            | 描述                                              |
| ------ | -------- | --------------- | ------------------------------------------------- |
| File   | 是       | String          | 文件 id/文件 url                                  |
| Sheets | 是       | Number/String[] | 数据表数组，支持两种形式，表名和索引（从 1 开始） |
...
合并表配置对象中删除数据源（只是编辑的本地对象，需要调 Apply 方法才能更新到云上）
...
| 参数名 | 是否必需 | 类型          | 描述                         |
| ------ | -------- | ------------- | ---------------------------- |
| Index  | 是       | Number/String | 支持索引、文件 id 和文件 url |
...
对特定的界面元素进行代理，代理方法返回 true 时，则不显示原界面
...
| 参数名 | 是否必需 | 类型   | 描述                                             |
| ------ | -------- | ------ | ------------------------------------------------ |
| CmbId  | 是       | string | 界面元素 ID，目前支持 `RecordInfo`(记录详情卡片) |
...
# 191 API 文档\_API_Sheet_Sheet 对象
...
| [FieldId](/documents/app-integration-dev/guide/dbsheet/Api/Sheet_FieldId.html)                 | 通过字段名称来获取字段 Id              |
| [GetFields](/documents/app-integration-dev/guide/dbsheet/Api/Sheet_GetFields.html)             | 获取该数据表的所有字段信息             |
| [Share](/documents/app-integration-dev/guide/dbsheet/Api/Sheet_Share.html)                     | sheet 分享                             |
...
# 192 API 文档\_API_Sheet_FieldDescriptors
...
# 196 API 文档\_API_Sheet_RecordRange
...
# 198 API 文档\_API_Sheet_AddDescription
...
# 199 API 文档\_API_Sheet_AppendFromCloud
...
# 200 API 文档\_API_Sheet_AppendFromLocal
...
# 202 API 文档\_API_Sheet_CreateFields
...
async function example() {
  await instance.ready();
  const app = instance.Application;
  await app.ActiveSheet.CreateFields([
    { name: "等级", type: "Rating", max: 5 },
    { name: "富文本", type: "MultiLineText" },
  ]);
}
```
...
```javascript
function main() {
  Application.ActiveSheet.CreateFields([
    { name: "等级", type: "Rating", max: 5 },
    { name: "富文本", type: "MultiLineText" },
  ]);
}
...
# 204 API 文档\_API_Sheet_DeleteFields
...
# 206 API 文档\_API_Sheet_GetFields
...
# 208 API 文档\_API_Sheet_UpdateFields
...
async function example() {
  ...
  await app
    .Sheets(1)
    .UpdateFields([{ id: "L", name: "富文本123", type: "MultiLineText" }]);
}
```
...
```javascript
function main() {
  Application.Sheets(1).UpdateFields([
    { id: "L", name: "富文本123", type: "MultiLineText" },
  ]);
}
main();
```

# 209 API 文档\_API_Sheet_OnCreateRecord
...
# 210 API 文档\_API_Sheet_OnDelete
...
# 211 API 文档\_API_Sheet_OnRename
...
# 212 API 文档\_API_Sheets_Sheets 对象
...
| [ItemById](/documents/app-integration-dev/guide/dbsheet/Api/Sheets_ItemById.html)                       | 通过 Id 获取 数据表            |
...
# 215 API 文档\_API_Sheets_GetActiveSheetId
...
# 216 API 文档\_API_Sheets_GetActiveSheetIndex
...
# 218 API 文档\_API_Sheets_ItemById
...
# 220 API 文档\_API_Sheets_OnCreateSheet
...
# 221 API 文档\_API_CalendarView_CalendarView 对象
...
# 222 API 文档\_API_CalendarView_BeginField
...
# 223 API 文档\_API_CalendarView_EndField
...
# 224 API 文档\_API_CalendarView_TimelineColor
...
# 225 API 文档\_API_CalendarView_TimelineColorFollowField
...
# 226 API 文档\_API_CalendarView_TimelineColorType
...
# 227 API 文档\_API_CalendarView_TitleField
...
# 228 API 文档\_API_CardViewUI_CardViewUI 对象
...
# 229 API 文档\_API_CardViewUI_ViewMode
...
# 230 API 文档\_API_GanttView_GanttView 对象
...
# 231 API 文档\_API_GanttView_BeginField
...
# 232 API 文档\_API_GanttView_Calendars
...
# 233 API 文档\_API_GanttView_EndField
...
# 234 API 文档\_API_GanttView_IsOnlyWorkDay
...
# 235 API 文档\_API_GanttView_TimelineColor
...
# 236 API 文档\_API_GanttView_TimelineColorFollowField
...
# 237 API 文档\_API_GanttView_TimelineColorType
...
# 238 API 文档\_API_GanttViewUI_GanttViewUI 对象
...
# 239 API 文档\_API_GanttViewUI_GanttGridFold
...
# 240 API 文档\_API_GanttViewUI_ViewMode
...
# 241 API 文档\_API_GanttViewUI_NextPage
...
# 242 API 文档\_API_GanttViewUI_PrevPage
...
# 243 API 文档\_API_GanttViewUI_Today
...
# 244 API 文档\_API_GridView_GridView 对象
...
# 245 API 文档\_API_GridView_FrozenCols
...
# 246 API 文档\_API_GridView_RowHeight
...
# 247 API 文档\_API_QueryView_QueryView 对象
...
# 248 API 文档\_API_QueryView_BackgroundImage
...
# 249 API 文档\_API_QueryView_QueryFields
...
| [Id](/documents/app-integration-dev/guide/dbsheet/Api/View_Id.html)                   | 视图 Id                            |          |
...
| [RecordRange](/documents/app-integration-dev/guide/dbsheet/Api/View_RecordRange.html) | 视图的 RecordRange                 |          |
| [Records](/documents/app-integration-dev/guide/dbsheet/Api/View_Records.html)         | 视图的 Records                     |          |
| [Selection](/documents/app-integration-dev/guide/dbsheet/Api/View_Selection.html)     | 该视图的选中的 RecordRange         |          |
...
# 251 API 文档\_API_View_Description
...
# 256 API 文档\_API_View_IsFavView
...
# 257 API 文档\_API_View_IsPersonal
...
# 259 API 文档\_API_View_RecordRange
...
# 261 API 文档\_API_View_Selection
...
# 264 API 文档\_API_View_ViewShare
...
# 270 API 文档\_API_ViewShare_ViewShare 对象
...
| [EditableFieldsInfo](/documents/app-integration-dev/guide/dbsheet/Api/ViewShare_EditableFieldsInfo.html)   | 返回当前分享视图高级权限中，允许编辑的字段 id | 可读写   |
...
# 271 API 文档\_API_ViewShare_CanAddRemoveRecords
...
# 272 API 文档\_API_ViewShare_EditableFieldsInfo
...
# 273 API 文档\_API_ViewShare_EditableRecType
...
# 274 API 文档\_API_ViewShare_ShareUrl
...
# 275 API 文档\_API_ViewShare_SharedLinkInfo
...
function main() {
  const sharedLinkInfo =
    Application.Sheets(1).Views(1).ViewShare.SharedLinkInfo;
}
main();
```

# 276 API 文档\_API_ViewShare_VisibleRecType
...
# 277 API 文档\_API_ViewShare_ChangePermission
...
# 278 API 文档\_API_ViewShare_SetEnable
...
| 参数名 | 是否必需 | 类型    | 描述                                          |
| ------ | -------- | ------- | --------------------------------------------- |
| Enable | 是       | boolean | 切换分享视图的开关，true 就是开，false 就是关 |
...
# 279 API 文档\_API_Views_Views 对象
...
| [ItemById](/documents/app-integration-dev/guide/dbsheet/Api/Views_ItemById.html) | 根据 视图 Id 获取视图    |
...
# 280 API 文档\_API_Views_ActiveView
...
| 参数名 | 是否必需 | 类型          | 描述                  |
| ------ | -------- | ------------- | --------------------- |
| Index  | 是       | number/string | 索引从 1 开始/视图 ID |
...
根据 索引或名称 获取视图 [View](/documents/app-integration-dev/guide/dbsheet/Api/View.html), 注意当获取到的视图类型不同时，返回的 View 对象也会不同，参看返回值
...
# 285 API 文档\_API_Views_ItemById
...
根据 视图 Id 获取视图 [View](/documents/app-integration-dev/guide/dbsheet/Api/View.html), 注意当获取到的视图类型不同时，返回的 View 对象也会不同，参看返回值
...
# 286 API 文档\_API_Views_OnCreate
...
# 287 API 文档\_API_Record_Record 对象
...
Record 对象，表示单条记录相关,Record 对象可以返回值的数组或者显示的文本数据，值的数组不同的字段会有不同的对象显示 常见对象如下：
...
# 294 API 文档\_API_RecordRange_RecordRange 对象
...
RecordRange 对象代表记录 Record,如果 RecordRange 在 Sheet 上返回，则此 RecordRange 指向的是 Sheet 上的数据，如果 RecordRange 在 View 上返回，则此 RecordRange 指向的是视图上的数据。我们为 RecordRange 提供了多种读取数据的方法。
...
5、通过参数 2 为字符串指定字段 ID 可以限定显示的字段

```javascript
RecordRange(1, "a");
```

6、通过参数 2 为数组可以指定多个字段 ID 可以限定显示的字段

```javascript
RecordRange(1, ["a", "b", "A"]);
```

7、通过参数 2 如果以@字符作为首字符表示字段名称
...
async function example() {
  await instance.ready();
  const app = instance.Application;
  const values = await app.ActiveView.RecordRange(1, [
    "@名称",
    "@日期",
    "@数量",
  ]).Value;
...
```javascript
function main() {
  const values = Application.ActiveView.RecordRange(1, [
    "@名称",
    "@日期",
    "@数量",
  ]).Value;
...
# 295 API 文档\_API_RecordRange_Count
...
返回当前 RecordRange 的记录数量，在 View 里返回 View 中可见记录数量，在 Sheet 返回所有的记录数量，如果指定了 RecordRange 的范围则返回 RecordRange 的记录数量
...
# 296 API 文档\_API_RecordRange_FieldId
...
返回指定 RecordRange 的字段 Id，如果 RecordRange 未指定数据，则返回 undefined
...
# 297 API 文档\_API_RecordRange_Font
...
# 298 API 文档\_API_RecordRange_Id
...
返回指定 RecordRange 的记录 id，如果 RecordRange 未指定数据，则返回 undefined
...
# 299 API 文档\_API_RecordRange_Index
...
返回指定 RecordRange 的记录索引，如果 RecordRange 未指定数据，则返回 undefined
...
# 300 API 文档\_API_RecordRange_Interior
...
# 301 API 文档\_API_RecordRange_Text
...
# 302 API 文档\_API_RecordRange_Value
...
# 303 API 文档\_API_RecordRange_Add
...
# 304 API 文档\_API_RecordRange_Condition
...
# 305 API 文档\_API_RecordRange_Delete
...
# 306 API 文档\_API_RecordRange_Item
...
| 参数名 | 是否必需 | 类型                | 描述                                                                                      |
| ------ | -------- | ------------------- | ----------------------------------------------------------------------------------------- |
| Index  | 否       | number/string/array | 传入 number 时索引从 1 开始，传入字符串时表示 id                                          |
| Field  | 否       | number/string/array | 传入 number 时索引从 1 开始，传入字符串时如果字符串以@开始则表示为字段名称，否则为字段 id |
...
async function example() {
  await instance.ready();
  const app = instance.Application;
  const values = await app.ActiveView.RecordRange.Item(1, [
    "@名称",
    "@日期",
    "@数量",
  ]).Value;
...
# 307 API 文档\_API_RecordRange_Select
...
# 308 API 文档\_API_RecordRange_SetValues
...
| ignoreErr | 否       | bool  | 默认值为 flase,当设置为 true 时，输入数据中某一条数据出错，这条数据不写入，其它数据正常写入；当设置为 false 时，其中一条数据出错，则都不写入 |
...
async function example() {
  await instance.ready();
  const Application = instance.Application;
  const res = await WPSOpenApi.Application.RecordRange("1:2", [
    "@名称",
    "@数量",
  ]).SetValues([["1111", 2222]], true);
  if (res.code !== 0) {
...
```javascript
function main() {
  const res = Application.RecordRange("1:2", ["@名称", "@数量"]).SetValues(
    [["1111", 2222]],
    true
  );
  if (res.code !== 0) {
...
# 309 API 文档\_API_RecordRange_OnDeleteRecord
...
# 310 API 文档\_API_RecordRange_OnUpdate
...
# 311 API 文档\_API_Records_Records 对象
...
Records 对象，表示记录相关集合，用户可通过 Records 对象直接访问记录和字段 支持以下几种访问方式 View.Records.Item(记录 id) View.Records.Item(记录索引) View.Records(记录 ID/记录索引, 字段索引/字段名称) View.Records(记录 ID/记录索引).Item(字段索引/字段名称)
...
async function example() {
  ...
  const insertRecords = await app
    .Sheets(1)
    .Views(1)
    .Records.Add(10, undefined, 2); // 在第10行，向上方添加2条记录
  // 输出 2
...
function main() {
  ...
  const insertRecords = Application.Sheets(1)
    .Views(1)
    .Records.Add(10, undefined, 2); // 在第10行，向上方添加2条记录
  // 输出 2
...
| 参数名 | 是否必需 | 类型          | 描述                                                            |
| ------ | -------- | ------------- | --------------------------------------------------------------- |
| Before | 否       | number/string | 指定的记录对象（索引从 1 开始/记录 ID），新建的记录将基于此之前 |
| After  | 否       | number/string | 指定的记录对象（索引从 1 开始/记录 ID），新建的记录将基于此之后 |
| Count  | 否       | number        | 插入多少条记录，默认值为 1 条                                   |
...
# 314 API 文档\_API_Records_Delete
...
# 315 API 文档\_API_Records_FindNext
...
# 316 API 文档\_API_Records_FindPrevious
...
# 318 API 文档\_API_AddressField_AddressField 对象
...
AddressField 地址字段的属性，修改属性之后需要 FieldDescriptor 调用 Apply()方法使得修改生效。如果非地址字段，无法设置相关属性
...
# 319 API 文档\_API_AddressField_IsDetailedAddress
...
# 320 API 文档\_API_AddressField_IsUsePresetAddress
...
# 321 API 文档\_API_AddressField_Level
...
# 322 API 文档\_API_AddressField_PresetAddress
...
# 323 API 文档\_API_Attachment_Attachment 对象
...
async function example() {
  await instance.ready();
  const app = instance.Application;
  const dbCellValue = await app.Sheets(1).Views(1).RecordRange(1, "@图片和附件")
    .Value;
...
```javascript
function main() {
  const dbCellValue = Application.Sheets(1)
    .Views(1)
    .RecordRange(1, "@图片和附件").Value;
  const attments = dbCellValue.Value;
...
# 324 API 文档\_API_Attachment_FileId
...
async function example() {
  await instance.ready();
  const app = instance.Application;
  const value = await app.Sheets(1).Views(1).RecordRange(1, "@图片和附件")
    .Value;
...
# 325 API 文档\_API_Attachment_FileName
...
# 326 API 文档\_API_Attachment_FileSize
...
# 327 API 文档\_API_Attachment_FileType
...
# 328 API 文档\_API_Attachment_ImgSize
...
# 329 API 文档\_API_Attachment_LinkUrl
...
# 330 API 文档\_API_AttachmentField_AttachmentField 对象
...
AttachmentField 图片和附件字段的属性，修改属性之后需要 FieldDescriptor 调用 Apply()方法使得修改生效。如果非图片和附件字段，无法设置相关属性
...
# 331 API 文档\_API_AttachmentField_DisplayStyle
...
# 332 API 文档\_API_AttachmentField_IsOnlyCameraUpload
...
# 333 API 文档\_API_AutoLinkCondition_AutoLinkCondition 对象
...
# 334 API 文档\_API_AutoLinkCondition_DateIntersectedValues
...
# 335 API 文档\_API_AutoLinkCondition_IntersectedConds
...
# 336 API 文档\_API_AutoLinkCondition_LinkSheetFieldId
...
单向关联和双向关联字段的自动匹配条件，关联的数据表字段 Id
...
# 337 API 文档\_API_AutoLinkCondition_OpType
...
# 338 API 文档\_API_AutoLinkCondition_SheetCondContents
...
# 339 API 文档\_API_AutoLinkCondition_SheetCondType
...
# 340 API 文档\_API_AutoLinkConditions_AutoLinkConditions 对象
...
# 341 API 文档\_API_AutoLinkConditions_Count
...
# 342 API 文档\_API_AutoLinkConditions_Add
...
| 参数名                | 是否必需 | 类型                   | 描述                   |
| --------------------- | -------- | ---------------------- | ---------------------- |
| LinkSheetFieldId      | 是       | string                 | 引用表的字段 ID        |
...
# 343 API 文档\_API_AutoLinkConditions_Delete
...
# 344 API 文档\_API_AutoLinkConditions_Item
...
# 345 API 文档\_API_AutoLinkGroup_AutoLinkGroup 对象
...
# 346 API 文档\_API_AutoLinkGroup_Conditions
...
# 347 API 文档\_API_AutoLinkGroups_AutoLinkGroups 对象
...
# 348 API 文档\_API_AutoLinkGroups_Count
...
```javascript
function main() {
  const autoLinkGroups =
    Applicaiton.Sheets(1).FieldDescriptors(2).AutoLinkGroups;
  const count = autoLinkGroups.Count;
}
main();
```

# 349 API 文档\_API_AutoLinkGroups_Add
...
# 350 API 文档\_API_AutoLinkGroups_Delete
...
# 351 API 文档\_API_AutoLinkGroups_Item
...
# 352 API 文档\_API_AutomationField_AutomationField 对象
...
AutomationField 自动任务字段的属性，修改属性之后需要 FieldDescriptor 调用 Apply()方法使得修改生效。如果非自动任务字段，无法设置相关属性
...
# 353 API 文档\_API_AutomationField_ContactField
...
# 354 API 文档\_API_AutomationField_ExecuteTime
...
# 355 API 文档\_API_AutomationField_TriggerField
...
自动任务字段的触发字段，当 Type 为 CheckedNotifyContact 时，触发字段是复选框字段，当 Type 为 DueDateNotifyContact 时，触发字段为日期字段
...
# 356 API 文档\_API_AutomationField_Type
...
# 357 API 文档\_API_ButtonField_ButtonField 对象
...
ButtonField 按钮字段的属性，修改属性之后需要 FieldDescriptor 调用 Apply()方法使得修改生效。如果非按钮字段，无法设置相关属性
...
function main() {
  const fieldDescriptor =
    Application.Sheets("数据表").FieldDescriptors("@按钮");
...
# 358 API 文档\_API_ButtonField_BackgroundColor
...
# 359 API 文档\_API_ButtonField_Icon
...
# 360 API 文档\_API_ButtonField_SuccessText
...
# 361 API 文档\_API_ButtonField_Text
...
# 362 API 文档\_API_ButtonField_TextColor
...
# 363 API 文档\_API_CascadeField_CascadeField 对象
...
CascadeField 级联字段的属性，修改属性之后需要 FieldDescriptor 调用 Apply()方法使得修改生效。如果非级联字段，无法设置相关属性 级联字段最多设置为 4 级
...
# 364 API 文档\_API_CascadeField_AllCascadeOption
...
# 365 API 文档\_API_CascadeField_IsDisplayAllLevel
...
# 366 API 文档\_API_CascadeField_Title
...
# 367 API 文档\_API_CascadeOption_CascadeOption 对象
...
# 368 API 文档\_API_CascadeOption_Children
...
# 369 API 文档\_API_CascadeOption_Id
...
async function example() {
  await instance.ready();
  const app = instance.Application;
  const cascadeOptions = await app.Sheets(1).FieldDescriptors(2)
    .AllCascadeOption;
...
function main() {
  const cascadeOptions =
    Application.Sheets(1).FieldDescriptors(2).AllCascadeOption;
  console.log(cascadeOptions.Item(1).Id);
}
main();
```

# 370 API 文档\_API_CascadeOption_Value
...
# 371 API 文档\_API_CascadeOptions_CascadeOptions 对象
...
# 372 API 文档\_API_CascadeOptions_Count
...
# 373 API 文档\_API_CascadeOptions_Add
...
# 374 API 文档\_API_CascadeOptions_Item
...
# 375 API 文档\_API_ContactField_ContactField 对象
...
ContactField 联系人字段的属性，修改属性之后需要 FieldDescriptor 调用 Apply()方法使得修改生效。如果非联系人字段，无法设置相关属性
...
# 376 API 文档\_API_ContactField_IsSupportMulti
...
# 377 API 文档\_API_ContactField_IsSupportNotice
...
# 378 API 文档\_API_DBCellValue_DBCellValue 对象
...
单元格数据，读取单元格值时， 如果是复杂的字段数据，则会返回 DBCellValue, 比如：多选项/超链接/联系人/级联字段/关联字段/地址字段/图片与附件。 当通过 RecordRange.Value 设置复杂的字段数据时，需要构造 DBCellValue 作为参数设置到单元格

```javascript
ActiveView.RecordRange([5, 6], "@地址").Value = DBCellValue({
  districts: ["广东省", "珠海市", "香洲区"],
  detail: "云海路",
});
```
...
# 379 API 文档\_API_DBCellValue_Value
...
返回单元格的值 地址字段类型: 通过 DBCellValue() 生成字段的数据
...
# 380 API 文档\_API_DateField_DateField 对象
...
DateField 日期字段的属性，修改属性之后需要 FieldDescriptor 调用 Apply()方法使得修改生效。如果非日期字段，无法设置相关属性
...
# 381 API 文档\_API_DateField_IsShowHoliday
...
# 382 API 文档\_API_DateField_IsShowTime
...
# 383 API 文档\_API_DateField_IsShowWeek
...
# 384 API 文档\_API_Field_Field 对象
...
# 385 API 文档\_API_Field_FieldDescriptor
...
# 393 API 文档\_API_FieldDescriptor_FieldDescriptor 对象
...
FieldDescriptor 描述了字段的属性，可以通过 SetType 修改字段的类型，修改属性之后需要主动调用 Apply()方法使得修改生效。 每次读取 FieldDescriptor 都会重新生成数据，所以修改前需要记录下当前的 FieldDescriptor，才能正确调用 Apply()
...
不同的字段类型有不同的属性设置，不是对应的字段类型，获取相关的属性时会返回 null，无法正常设置
...
# 394 API 文档\_API_FieldDescriptor_DefaultVal
...
# 395 API 文档\_API_FieldDescriptor_DefaultValType
...
# 396 API 文档\_API_FieldDescriptor_Description
...
# 397 API 文档\_API_FieldDescriptor_Id
...
# 398 API 文档\_API_FieldDescriptor_IsSyncField
...
# 399 API 文档\_API_FieldDescriptor_IsValueUnique
...
# 400 API 文档\_API_FieldDescriptor_Name
...
# 401 API 文档\_API_FieldDescriptor_NumberFormat
...
# 402 API 文档\_API_FieldDescriptor_Type
...
# 403 API 文档\_API_FieldDescriptor_Apply
...
# 404 API 文档\_API_FieldDescriptor_Delete
...
# 405 API 文档\_API_FieldDescriptor_OnDelete
...
# 406 API 文档\_API_FieldDescriptor_OnUpdate
...
# 407 API 文档\_API_FieldDescriptors_FieldDescriptors 对象
...
function main() {
  ...
  // 创建关联字段到sheet2
  const linkField = Application.FieldDescriptor(
    Enum.DbSheetFieldType.OneWayLink,
    "关联字段到sheet2"
  );
...
*/
  ...
  // 创建引用字段
  const descriptor = Application.FieldDescriptor(
    Enum.DbSheetFieldType.Lookup,
    "引用关联字段到sheet2"
  );
...
# 408 API 文档\_API_FieldDescriptors_Count
...
# 409 API 文档\_API_FieldDescriptors_AddField
...
# 410 API 文档\_API_FieldDescriptors_Delete
...
| 参数名             | 是否必需 | 类型    | 描述                                                |
| ------------------ | -------- | ------- | --------------------------------------------------- |
| FieldId            | 是       | string  | 字段 Id,可以通过字段名获取 Sheet.FieldId(fieldName) |
...
# 411 API 文档\_API_FieldDescriptors_FieldDescriptor
...
| 参数名 | 是否必需 | 类型   | 描述                   |
| ------ | -------- | ------ | ---------------------- |
| Type   | 是       |        | FieldDescriptor 的类型 |
| Name   | 是       | string | FieldDescriptor 的名称 |
...
# 412 API 文档\_API_FieldDescriptors_Item
...
返回字段的信息，可以通过 FieldDescriptors.Item() 返回某个字段的信息，也可以简化使用 FieldDescriptors(Index) 来返回字段信息。
...
| 参数名 | 是否必需 | 类型          | 描述                                                                                        |
| ------ | -------- | ------------- | ------------------------------------------------------------------------------------------- |
| Index  | 否       | string/number | Index 为 number 类型时表示当前字段的索引，Index 为字段串类型的时候表示字段的 Id 或@字段名称 |
...
# 413 API 文档\_API_FieldDescriptors_OnCreate
...
# 414 API 文档\_API_Fields_Fields 对象
...
网格视图列的集合，可以访问视图的列属性，通过 Item 方法访问列时，注意列如果是隐藏了就无法通过索引获取，只能通过名称或者字段 ID 来获取
...
返回视图字段的信息，可以通过 Fields.Item() 返回某个字段的信息，也可以简化使用 Fields(Index) 来返回字段信息。通过 Item 方法访问列时，注意列如果是隐藏了就无法通过索引获取，只能通过名称或者字段 ID 来获取。
...
| 参数名 | 是否必需 | 类型          | 描述                                                                                                               |
| ------ | -------- | ------------- | ------------------------------------------------------------------------------------------------------------------ |
| Index  | 否       | string/number | Index 为 number 类型时表示当前字段的索引，Index 为字段串类型的时候, 如果是以@开头则表示是字段名，否则表示字段的 Id |
...
# 417 API 文档\_API_FormulaField_FormulaField 对象
...
FormulaField 公式字段的属性，修改属性之后需要 FieldDescriptor 调用 Apply()方法使得修改生效。如果非公式字段，无法设置相关属性
...
# 418 API 文档\_API_FormulaField_Formula
...
# 419 API 文档\_API_FormulaField_IsShowPercentAsProgress
...
公式字段 “将百分比显示为进度” 属性, 只有当字段属性 ValueType 设置为
...
# 420 API 文档\_API_FormulaField_ValueType
...
# 421 API 文档\_API_LinkField_LinkField 对象
...
LinkField 关联字段的属性，修改属性之后需要 FieldDescriptor 调用 Apply()方法使得修改生效。如果非关联字段，无法设置相关属性
...
async function example() {
  ...
  const linkFieldId = await Application.Sheets("数据表").FieldDescriptors(
    "@单向关联"
  );
...
# 422 API 文档\_API_LinkField_AutoLinkGroups
...
# 423 API 文档\_API_LinkField_IsAutoLink
...
# 424 API 文档\_API_LinkField_IsSupportMultiLinks
...
# 425 API 文档\_API_LinkField_LinkSheet
...
# 426 API 文档\_API_LinkField_LinkView
...
# 427 API 文档\_API_LookupField_LookupField 对象
...
LookupField 引用/查找引用/统计 字段的属性，修改属性之后需要 FieldDescriptor 调用 Apply()方法使得修改生效。如果非 引用/查找引用/统计 字段，无法设置相关属性
...
# 428 API 文档\_API_LookupField_LinkFieldId
...
# 429 API 文档\_API_LookupField_LookupFieldId
...
引用内容里 引用字段的 Id，指向的是引用的数据表的字段（不一定是当前数据表）,需要先从关联的表中读取字段 Id。
...
function main() {
  const fieldDescriptor =
    Application.Sheets("数据表").FieldDescriptors("@查找引用");
  const prop = fieldDescriptor.Lookup;
...
# 430 API 文档\_API_LookupField_LookupSheetId
...
引用字段里 引用 sheet 的 Id，要通过 Sheet Id 获取 Sheet 的实例可以通过 Application.Sheets.ItemById(id)
...
# 431 API 文档\_API_LookupField_LookupType
...
引用字段的显示方法,查找引用字段不允许配置，只能使用 Origin，统计字段不允许设置 Origin
...
# 432 API 文档\_API_NumberField_NumberField 对象
...
NumberField 数字字段的属性，修改属性之后需要 FieldDescriptor 调用 Apply()方法使得修改生效。如果非数字字段，无法设置相关属性
...
# 433 API 文档\_API_NumberField_IsShowThousand
...
# 434 API 文档\_API_RatingField_RatingField 对象
...
RatingField 等级字段的属性，修改属性之后需要 FieldDescriptor 调用 Apply()方法使得修改生效。如果等级字段，无法设置相关属性
...
# 435 API 文档\_API_RatingField_MaxRating
...
# 436 API 文档\_API_SelectField_SelectField 对象
...
SelectField 单选项/多选项字段的属性，修改属性之后需要 FieldDescriptor 调用 Apply()方法使得修改生效。如果单选项/多选项字段，无法设置相关属性
...
# 437 API 文档\_API_SelectField_IsAddItemWhenInputting
...
# 438 API 文档\_API_SelectField_Items
...
# 439 API 文档\_API_TextLinkRun_TextLinkRun 对象
...
# 440 API 文档\_API_TextLinkRun_Address
...
# 441 API 文档\_API_TextLinkRun_Length
...
# 442 API 文档\_API_TextLinkRun_LinkRunsType
...
# 443 API 文档\_API_TextLinkRun_Pos
...
# 444 API 文档\_API_TextLinkRuns_TextLinkRuns 对象
...
# 445 API 文档\_API_TextLinkRuns_Count
...
# 446 API 文档\_API_TextLinkRuns_Item
...
| 参数名 | 是否必需 | 类型   | 描述                        |
| ------ | -------- | ------ | --------------------------- |
| Index  | 否       | number | 传入 number 时索引从 1 开始 |
...
# 447 API 文档\_API_UrlField_UrlField 对象
...
UrlField 超链接字段的属性，修改属性之后需要 FieldDescriptor 调用 Apply()方法使得修改生效。如果非超链接字段，无法设置相关属性
...
# 448 API 文档\_API_UrlField_HyperLinkText
...
# 449 API 文档\_API_WatchedField_WatchedField 对象
...
WatchedField 最后修改人/最后修改时间 字段的属性，修改属性之后需要 FieldDescriptor 调用 Apply()方法使得修改生效。如果最后修改人/最后修改时间，无法设置相关属性
...
# 450 API 文档\_API_WatchedField_IsWatchedAll
...
# 451 API 文档\_API_WatchedField_WatchedFields
...
# 454 API 文档\_API_Sort_IsAscending
...
# 456 API 文档\_API_Sorts_Sorts 对象
...
# 460 API 文档\_API_Sorts_ChangeOrder
...
# 462 API 文档\_API_Criteria_Criteria 对象
...
# 463 API 文档\_API_Criteria_Field
...
# 465 API 文档\_API_Criteria_Value
...
各筛选规则独立地限制了 values 数组内最多允许填写的元素数，当 values 内元素数超过阈值时，该筛选规则将失效。“为空、不为空”只允许在 op 为“Intersected”时填写。“介于”允许最多填写 2 个元素；“指定值”允许填写 65535 个元素；其他规则允许最多填写 1 个元素 values[]数组内的元素为字符串时，表示文本匹配。目前还支持对日期进行动态筛选，此时 values[]内的元素需以结构体的形式给出：
...
# 466 API 文档\_API_Filter_Filter 对象
...
# 467 API 文档\_API_Filter_Criteria
...
# 468 API 文档\_API_Filter_FieldId
...
# 469 API 文档\_API_Filter_FilterId
...
# 471 API 文档\_API_Filters_Filters 对象
...
# 473 API 文档\_API_Filters_Operator
...
# 477 API 文档\_API_Group_Group 对象
...
# 479 API 文档\_API_Group_IsAscending
...
# 482 API 文档\_API_Groups_Groups 对象
...
# 484 API 文档\_API_Groups_IsTempUnFold
...
# 486 API 文档\_API_Groups_FoldAll
...
| 参数名 | 是否必需 | 类型          | 描述                                 |
| ------ | -------- | ------------- | ------------------------------------ |
| Index  | 是       | number/string | 获取的分组字段索引/获取的分组字段 ID |
...
# 488 API 文档\_API_Groups_UnFoldAll
...
# 489 API 文档\_API_DbComment_DbComment 对象
...
# 491 API 文档\_API_DbComment_Text
...
# 492 API 文档\_API_DbComment_Delete
...
# 493 API 文档\_API_RecordComment_RecordComment 对象
...
RecordComment 对象代表记录 Record 的评论，可以通过 RecordComment 对象对记录的评论进行操作。
...
# 494 API 文档\_API_RecordComment_Count
...
# 495 API 文档\_API_RecordComment_Add
...
# 496 API 文档\_API_RecordComment_Delete
...
# 497 API 文档\_API_RecordComment_Item
...
| 参数名 | 是否必需 | 类型          | 描述                                                 |
| ------ | -------- | ------------- | ---------------------------------------------------- |
| Index  | 否       | number/string | 传入 number 时索引从 1 开始，传入字符串时表示评论 id |
...
# 498 API 文档\_API_RecordComments_RecordComments 对象
...
RecordComments 当前视图上的评论，每条记录对应一个 RecordComments 对象，RecordComments 对象的属性和方法可以用来操作该条记录的评论。 多维表的评论是在记录上的，评论的对象主体上是三级 RecordComments -&gt; RecordComment -&gt; DbComment
...
# 499 API 文档\_API_RecordComments_Count
...
返回当前 RecordComment 的数量，因为每条记录都会有一个 RecordComment 对象，所以返回的是记录的数量。
...
# 500 API 文档\_API_RecordComments_Item
...
通过记录的索引或 ID 获取指定记录的评论对象。可以简化写法 RecordComments.Item(1) 可以简化为 RecordComments(1)
...
| 参数名 | 是否必需 | 类型          | 描述                                                     |
| ------ | -------- | ------------- | -------------------------------------------------------- |
| Index  | 否       | number/string | 传入 number 时记录索引从 1 开始，传入字符串时表示记录 id |
...
# 501 API 文档\_API_RecordComments_OnCreate
...
# 502 API 文档\_API_RecordComments_OnDelete
...
# 503 API 文档\_API_DataSource_DataSource 对象
...
# 504 API 文档\_API_DataSource_CreateSummarySheet
...
# 505 API 文档\_API_DataSource_CreateSyncDBSheets
...
# 506 API 文档\_API_DataSource_ImportFromCloud
...
# 507 API 文档\_API_DataSource_ImportFromLocal
...
# 508 API 文档\_API_SummarySheet_SummarySheet 对象
...
# 509 API 文档\_API_SummarySheet_SourceConfigs
...
# 510 API 文档\_API_SummarySheet_RefreshSyncSheet
...
# 511 API 文档\_API_SummarySheet_RemoveSheetSyncLink
...
# 512 API 文档\_API_SummarySourceConfig_SummarySourceConfig 对象
...
合并表数据源对象中的某个源文件配置对象，数据源对象是由多个文件配置组成的一个数组。 每个源文件配置对象都包含了两个属性，文件 id：FileId，文件中选中的数据表 id 数组：SheetIds
...
# 513 API 文档\_API_SummarySourceConfig_FileId
...
可读写 合并表中某个源文件配置对象的文件 id。注意：修改后，该对象中对应的 SheetIds 会自动重置为空数组。（只是编辑的本地对象，需要调 SummarySourceConfigs 对象中的 Apply 方法才能更新到云上）
...
# 514 API 文档\_API_SummarySourceConfig_SheetIds
...
可读写 合并表中某个源文件配置对象的选中数据表的 id 数组。（只是编辑的本地对象，需要调 SummarySourceConfigs 对象中的 Apply 方法才能更新到云上）
...
# 515 API 文档\_API_SummarySourceConfig_SetSheets
...
通过一组数据表名或索引，来修改合并表源文件配置对象中的 SheetIds 属性（只是编辑的本地对象，需要调 SummarySourceConfigs 对象中的 Apply 方法才能更新到云上）
...
# 516 API 文档\_API_SummarySourceConfig_SetUrl
...
通过 url 修改合并表源文件配置对象中的 FileId 属性（只是编辑的本地对象，需要调 SummarySourceConfigs 对象中的 Apply 方法才能更新到云上）
...
# 517 API 文档\_API_SummarySourceConfigs_SummarySourceConfigs 对象
...
# 518 API 文档\_API_SummarySourceConfigs_Count
...
# 519 API 文档\_API_SummarySourceConfigs_Add
...
# 520 API 文档\_API_SummarySourceConfigs_Apply
...
# 521 API 文档\_API_SummarySourceConfigs_Delete
...
# 522 API 文档\_API_SummarySourceConfigs_Item
...
| 参数名 | 是否必需 | 类型          | 描述                  |
| ------ | -------- | ------------- | --------------------- |
| Index  | 是       | number/string | 索引从 1 开始/文件 id |
...
# 523 API 文档\_API_SyncDBSheet_SyncDBSheet 对象
...
# 524 API 文档\_API_SyncDBSheet_RefreshSyncSheet
...
# 525 API 文档\_API_SyncDBSheet_RemoveSheetSyncLink
...
# 526 API 文档\_API_NoticeBar_NoticeBar 对象
...
# 527 API 文档\_API_NoticeBar_Visible
...
# 528 API 文档\_API_ApiResult_ApiResult 对象
...
# 529 API 文档\_API_ApiResult_Code
...
# 530 API 文档\_API_ApiResult_Message
...
# 531 API 文档*API*枚举值\_DbAutomationPresetType
...
# 532 API 文档*API*枚举值\_DbButtonIcon
...
| 枚举值                       | 字符串值            | 描述 |
| ---------------------------- | ------------------- | ---- |
| DbButtonIcon.GestureThumb    | 'gesture_thumb'     |      |
| DbButtonIcon.PaperPlane      | 'paper_plane'       |      |
...
| 枚举值                       | 字符串值      | 描述   |
| ---------------------------- | ------------- | ------ |
| DbFieldValueType.Fvt_Text    | 'Fvt_Text'    | 文本   |
| DbFieldValueType.Fvt_Number  | 'Fvt_Number'  | 数字   |
| DbFieldValueType.Fvt_Contact | "Fvt_Contact" | 联系人 |
| DbFieldValueType.Fvt_Date    | "Fvt_Date"    | 日期   |
| DbFieldValueType.Fvt_Time    | "Fvt_Time"    | 时间   |
| DbFieldValueType.Fvt_Logic   | "Fvt_Logic"   | 逻辑   |

# 534 API 文档*API*枚举值\_DbFilterCriteriaOpType
...
# 535 API 文档*API*枚举值\_DbGroupUnit
...
# 536 API 文档*API*枚举值\_DbLookupFunction
...
# 537 API 文档*API*枚举值\_DbSharedCriteriaType
...
# 538 API 文档*API*枚举值\_FilterOpType
...
# 539 API 文档*API*枚举值\_SharedLinkPermissionType
...
# 540 API 文档*API*枚举值\_SharedLinkToType
...
# 543 API 文档\_API_Interior_Interior 对象
...
# 544 API 文档\_API_Interior_Color
...
# 545 API 文档\_API_Navigator_Navigator 对象
...
# 546 API 文档\_API_Navigator_Visible
...
# 547 API 文档\_API_Window_Window 对象
...
窗口对象，窗口对象只能在浏览器内使用，在脚本编辑器由于是在服务器上运行的脚本，不适用于 Window 对象
...
# 548 API 文档\_API_Window_GanttViewUI
...
返回甘特视图的界面设置，如果当前视图不是甘特视图，则返回 undefined
...
# 549 API 文档\_API_Window_Navigator
...
# 550 API 文档\_API_Window_NoticeBar
...
# 551 API 文档\_API_Window_BailHook
...
# 552 API 文档\_API_Window_DisplayRecord
...
# 553 API 文档\_API_Window_HiddenAllRecord
...
# 554 API 文档\_API_Window_SetLayout
